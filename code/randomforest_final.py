...
def fit_predict(train_df, eval_df, model_factory=make_rf):
    model = model_factory()
    model.fit(train_df[FEATURES], train_df[TARGET])
    return model.predict(eval_df[FEATURES])


def in_regime_cv(regime, model_factory=make_rf, n_splits=N_SPLITS):
...
def run_experiment(train_regimes, test_regime, model_factory=make_rf):
...
print("saved results_%s_*.csv" % MODEL_NAME)
print(rob_df.to_string())
print(f"\nCRS (Random Forest, high-vol tests): {CRS:.4f}")
